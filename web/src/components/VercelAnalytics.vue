...
onMounted(() => {
  // Vercel Analytics nur in Production und sicher laden
  if (import.meta.env.PROD && typeof window !== 'undefined') {
    try {
      ...
      script.onerror = (error) => {
        console.warn('⚠️ Vercel Analytics Script konnte nicht geladen werden:', error)
      }
      
      document.head.appendChild(script)
    } catch (error) {
      console.warn('⚠️ Vercel Analytics konnte nicht initialisiert werden:', error)
    }
  }
})
</script>
